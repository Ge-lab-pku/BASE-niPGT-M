...
def calibrate(data, chrom, true_pos, window_size, sample, parent, err_rate, mcc_rate, ADO_thres, LFLoH_thres, recom_probs, df_save, df_save_path, flag, rec_rate_pd, recom_rate, sex = None):
    ...
    if chrom != "X" or (chrom == "X" and sex == "Female"):
        ...
        while True:
            ...
            df = LFLoH(new_truncated_data, LFLoH_thres, mcc_rate, err_rate, recom_probs, chrom, rec_rate_pd, recom_rate)
...
def LFLoH(data, LFLoH_thres, mcc_rate, err_rate, recom_probs, chrom, rec_rate_pd, recom_rate):
    ...
    rec_rate_pd: the documents to calculate the recombination probability.
    '''
            ...
            if crt_label == "F1" or crt_label == "F2":
                ...
                if max_label == "M" and prob_dict2[max_label] >= LFLoH_thres[max_label]:
                    ...
                    if crt_upper_index - 1 > upper_index:
                        recom_probs.update(all_recom_prob(df.iloc[np.r_[crt_upper_index-1,crt_upper_index+1:crt_index+1]], chrom, rec_rate_pd, recom_rate, output = False))
            ...
            if crt_label == "F1" or crt_label == "F2":
                ...
                if max_label == "M" and prob_dict2[max_label] >= LFLoH_thres[max_label]:
                    ...
                    if crt_lower_index + 1 < lower_index:
                        recom_probs.update(all_recom_prob(df.iloc[np.r_[crt_index:crt_lower_index,crt_lower_index+1]], chrom, rec_rate_pd, recom_rate, output = False))
